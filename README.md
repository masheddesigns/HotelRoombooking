...
- **Optional availability / capacity features:** both bonuses are implemented. Availability uses the half-open interval model `[checkIn, checkOut)` against demo bookings in `src/data/bookings.ts`, so a request starting exactly on an existing check-out day is available; invalid requested ranges are treated as unavailable so they can never pass the check. Capacity filtering uses the single `isRoomCapacitySufficient` rule, and a room only counts as selected while its dates, availability, and capacity all hold.
